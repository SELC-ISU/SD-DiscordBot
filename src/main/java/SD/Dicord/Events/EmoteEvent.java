...
public class EmoteEvent extends ListenerAdapter   {
	public void onMessageReceived(MessageReceivedEvent image)
	{
		...
		else if(image.getMessage().getContentRaw().equalsIgnoreCase(Variables.getPrefix() + "sad")) {
			emote = new EmbedBuilder();
		...
		else if(image.getMessage().getContentRaw().equalsIgnoreCase(Variables.getPrefix() + "happy")) {
			emote = new EmbedBuilder();
